...
def connect(username, password, outcodeemplacement):
    print("e")
    uName = username
    uPass = password
    ...
    with open(outcodeemplacement, 'w') as f:
        f.write(conv_err)
...
def getEDT(username, password, storeityesorno, storeityesemplacement, studentID):
    if os.path.exists(storeityesemplacement):
        os.remove(storeityesemplacement)
    print("e")
    uName = username
    ...
    conn.request("POST", "/v3/E/" + studentID + "/emploidutemps.awp?verbe=get", payload, headers)

    r2 = conn.getresponse()
    edtData = r2.read()
    conv_edt = str(edtData)
...
def getID(username, password, storeityesorno, storeityesemplacement):
    if os.path.exists(storeityesemplacement):
        os.remove(storeityesemplacement)
    print("e")
    uName = username
    ...
    ID = json.loads(data.decode("utf-8"))['data']['accounts'][0]['profile']['id']
    if storeityesorno == "yes":
        with open(storeityesemplacement, 'w') as f:
            f.write(ID)
